...
# Left Prompt
PROMPT='$(common_host)$(common_return_status)┌ %~$(common_bg_jobs)$(common_git_status)${_newline}$(common_return_status)$COMMON_PROMPT_SYMBOL%f '

# Right Prompt
RPROMPT='%{${_lineup}%}$(common_time)%{${_linedown}%}'
# Enable redrawing of prompt variables
setopt promptsubst

# Prompt with current SHA
# PROMPT='$(common_host)$(common_dir)$(common_bg_jobs)$(common_return_status)'
...
# Time
common_time() {
    echo -n "%{$fg[$COMMON_COLORS_TIME]%}$(date '+%R %a')"
}
...
common_return_status() {
  echo -n "%(?.%F{$COMMON_COLORS_RETURN_STATUS_TRUE}.%F{$COMMON_COLORS_RETURN_STATUS_FALSE})"
}
